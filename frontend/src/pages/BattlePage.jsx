...
export default function BattlePage({ raidRoomCode = null }) {
  ...
  const [passageChunkIdx, setPassageChunkIdx] = useState(0);
              ...
              {challenge.type === "passage_study" && (
                <div data-testid="passage-study" className="space-y-4">
                  ...
                  {(() => {
                    const chunks = challenge.chunks?.length ? challenge.chunks : [challenge.passage];
                    const chunk = chunks[Math.min(passageChunkIdx, chunks.length - 1)];
                    const isLast = passageChunkIdx >= chunks.length - 1;
                    return (
                      <>
                        <div className="rounded-2xl bg-violet-500/10 border border-violet-400/30 p-5 text-center">
                          <p className="text-[10px] uppercase tracking-widest text-violet-200/70 mb-3">
                            {t("passage_study_chunk", { n: passageChunkIdx + 1, total: chunks.length })}
                          </p>
                          <p className="text-base sm:text-lg leading-relaxed kaiti whitespace-pre-wrap">{chunk}</p>
                          <button type="button" onClick={() => { sfx.click(); speak(chunk); }} className="mt-4 text-sm text-violet-200 underline">{t("battle_read_along_listen")}</button>
                        </div>
                        <Button
                          onClick={() => {
                            sfx.click();
                            if (isLast) onCorrect();
                            else setPassageChunkIdx((i) => i + 1);
                          }}
                          disabled={!!feedback}
                          className="w-full rounded-2xl py-5 font-bold bg-violet-500 text-white"
                        >
                          {isLast ? t("passage_study_done") : t("passage_study_next")}
                        </Button>
                      </>
                    );
                  })()}
