...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    api.get("/home-status").then((r) => setDataConsent(r.data?.data_consent)).catch(() => {});
    ...
    <div className="min-h-screen bg-slate-950 text-white px-4 py-8" data-testid="settings-page">
      <div className="max-w-md mx-auto">
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-2xl bg-white/5 border border-white/10 p-4" data-testid="data-consent-setting">
            <div className="text-sm text-white/80 mb-2">{t("consent_required_title")}</div>
            {dataConsent?.can_collect_learning_data ? (
              <p className="text-sm text-emerald-300/90" data-testid="consent-active">{t("settings_consent_active")}</p>
            ) : dataConsent?.status === "pending" ? (
              <p className="text-sm text-amber-200/90" data-testid="consent-pending">
          ...
          <div className="rounded-2xl bg-white/5 border border-white/10 p-4">
            ...
            {sent ? (
              <p className="text-sm text-sky-200/80" data-testid="parent-invite-sent">
                {t("settings_invite_sent_detail")}
              </p>
            ) : (
              <>
                <Input
          ...
          <a href="/privacy" className="block text-center text-xs text-sky-200/60 underline py-2" data-testid="settings-privacy-link">
            Privacy Policy / 私隱政策
          </a>
        </div>
      </div>
