...
export function buildInviteShareText(publicUserId, baseUrl = APP_BASE) {
  ...
  return `Join me on Cram Journey! 🎮 Add my friend ID: ${publicUserId}\n${url}`;
}
...
export function buildVictoryShareText(result, publicUserId) {
  ...
  if (result.bossDefeated) {
    return streak >= 3
      ? `🔥 ${streak}-day streak! I just defeated ${boss} with ${score} points!${idSuffix}`
      : `🏆 I just defeated ${boss} with ${score} points!${idSuffix}`;
  }
  return `⚔️ Training hard — scored ${score} points battling ${boss}.${idSuffix}`;
}
