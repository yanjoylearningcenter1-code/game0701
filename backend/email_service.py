...
from brand import APP_NAME_EN, APP_NAME_ZH, PARENT_BRAND_EN, PARENT_BRAND_ZH
...
async def send_family_link_invite_email(parent_email: str, confirm_url: str) -> bool:
    subject = f"Link your child's {APP_NAME_EN} account — {PARENT_BRAND_EN}"
    html = f"""
    <div style="font-family:sans-serif;max-width:560px;margin:0 auto">
      <h2>Link your child's account</h2>
      <p>Your child asked to link your email to their <strong>{APP_NAME_EN}</strong> account
        ({PARENT_BRAND_EN}).</p>
      ...
      <h3>連結子女帳戶（繁體中文）</h3>
      <p>您的子女希望將您的電郵連結至 <strong>{APP_NAME_ZH}（{PARENT_BRAND_ZH}旗下學習 App）</strong>。</p>
...
async def send_consent_email(parent_email: str, confirm_url: str, consent_type: str) -> bool:
    ...
    subject = f"Confirm parental consent — {APP_NAME_EN} ({label})"
    html = f"""
    <div style="font-family:sans-serif;max-width:560px;margin:0 auto">
      <h2>Parental consent required</h2>
      <p>Please confirm <strong>{label}</strong> consent for your child's
        <strong>{APP_NAME_EN}</strong> account ({PARENT_BRAND_EN}).</p>
      <p>By confirming, you agree to our privacy policy:
        <a href="{PRIVACY_POLICY_URL}">{PRIVACY_POLICY_URL}</a></p>
...
async def send_progress_digest(
    ...
    subject = f"Your child's {APP_NAME_EN} progress ({freq_label})"
    html = f"""
    <div style="font-family:sans-serif;max-width:560px;margin:0 auto">
      <h2>{APP_NAME_EN} progress</h2>
      <ul>
      ...
      <hr style="border:none;border-top:1px solid #e2e8f0;margin:24px 0"/>
      <h3>{APP_NAME_ZH} 學習進度摘要（{PARENT_BRAND_ZH}）</h3>
      <ul>
